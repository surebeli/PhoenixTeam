...
- **User source documents are READ-ONLY** — never modify any file outside `.spec/`.
...
- **Identity persistence**: The current user's member code is read via `git config spec.member-code` (written to `.git/config` during init, machine-local, never committed). `.spec/COLLABORATORS.md` is the shared registry of all collaborators — never use it to derive current identity.
- **Identity guard**: Before executing any skill, read `git config spec.member-code`. If empty, stop immediately and output: `⚠️ No identity bound on this machine. Please run spec-whoami to bind your identity first.`
- **Branch guard** (all skills except spec-init): After the identity guard, run `git branch --show-current` and compare with `git config spec.main-branch`. If they differ, stop immediately and output: `❌ Current branch '{current}' is not the SpecTeam main branch '{main}'. SpecTeam operations must run on the main branch. Switch with: git checkout {main}`. spec-init is exempt — it establishes the main branch record. If `spec.main-branch` is not set (not yet initialized), skip the check.
...
- Run `git diff -- .spec/` before every push and output a summary; also check DIVERGENCES.md for open/proposed divergences.
- Two repo modes (set during init): Mode A (dedicated branch `spec-docs`, default) or Mode B (git submodule).
- Directory depth limit: max 2 levels (e.g. `.spec/design/alice/`, `.spec/archive/`).
...
**Step 0 — Determine mode**: Check if `.spec/` exists. Does not exist = founder mode. Exists = join mode.

**Step 1 — Record main branch**:
- Run `git branch --show-current`, capture as `{main_branch}`.
- Run `git config --local spec.main-branch {main_branch}`, write to local `.git/config`.
- **Founder mode**: This branch becomes the protected SpecTeam main branch; written to COLLABORATORS.md (see step 6).
- **Join mode**: Read the `Main Branch` field from existing `COLLABORATORS.md`, then run `git config --local spec.main-branch {main_branch}` to sync locally.
...
> You are the first to initialize SpecTeam. Please briefly describe the collaboration goal / mission (1–3 sentences). This will be written to THESIS.md as the North Star for all collaborators.
...
5. **Join mode**: Read `.spec/THESIS.md` and display the current project goal for the user to review.

6. Create `.spec/` and update `COLLABORATORS.md` (with `Main Branch` metadata, member code, directory mapping, collaborator list):
   ```markdown
   ...
   **Main Branch**: {main_branch}
   ...
   ```
7. Copy and normalize source documents to `.spec/design/{code}/` (max 2-level structure, prepend `<!-- Spec Normalized Document -->` header).
...
9. Run `git config --local spec.member-code {code}`, write member code to local `.git/config`.
10. `git add .spec/` and commit.
11. Auto-trigger **parse** skill.
12. Output completion message, explicitly state the protected main branch:
    `"Initialization complete! SpecTeam main branch locked to '{main_branch}'. All spec-* operations on other branches will be rejected."`
...
Execution steps:
1. Read `git config spec.member-code`.
2. If not empty, display current identity and information from COLLABORATORS.md.
3. If empty (new machine / not bound), read `.spec/COLLABORATORS.md`, list known collaborators, ask user to choose or create a new code.
4. Run `git config --local spec.member-code {chosen code}` to complete binding.
...
3. Run `git diff HEAD~1..HEAD -- .spec/` and output summary (grouped by member code).
...
1. Read `.spec/DIVERGENCES.md`, classify:
...
1. Run `git status`.
2. Run `git diff -- .spec/` and output **[Diff Summary]** grouped by member code.
3. `git add .spec/**/*.md` (plus DIVERGENCES.md, last-review.json, etc.).
4. Commit (default: `"[SpecTeam] {code} document update"`).
5. Push.
...
Execution steps:
1. Apply identity guard. Apply branch guard.
2. Scan `.spec/design/` (by member code subdirectories).
3. Update `.spec/INDEX.md` (directory tree + per-collaborator summaries + THESIS quote + SIGNALS).
4. Run `git log --oneline -3 -- .spec/` and `git diff HEAD~1..HEAD -- .spec/` (if commits exist), generate diff summary grouped by member code.
...
Execution steps:
1. Apply identity guard. Apply branch guard.
2. Run `git diff HEAD~5..HEAD -- .spec/` (recent changes by member code).
...
Execution steps:
1. Apply identity guard. Apply branch guard.
2. Run the corresponding git diff (default: `HEAD~1..HEAD -- .spec/`).
...
1. Read `.spec/last-review.json` (if exists) — contains each collaborator's commit hash at last review.
2. For each collaborator, run `git log --oneline -1 -- .spec/design/{code}/` to get their latest commit.
...
`"[SpecTeam] review — {N} new divergences, {M} known divergences"`
...
1. Read `.spec/DIVERGENCES.md`. No file or no actionable items → output "No divergences to handle. Run review first if needed." and stop.
...
4. After user chooses:
   ...
   - Commit: `"[SpecTeam] align — D-{N}: {title} proposed by {code}, awaiting {other}"`
...
1. **Generate Action Items**: Analyze the decision against each party's current documents (`.spec/design/`). For each collaborator requiring changes, generate a detailed instruction block (model-readable first, human-readable second). Show the status table and per-party instruction blocks to the user. If a party's document already aligns → mark "No changes needed", omit their instruction block.
2. **Create `.spec/decisions/D-{N}.md`**: Write full decision + per-party instruction blocks (with acceptance criteria). This is the ready-to-use file users pass to their own model to execute changes, and the authoritative source for update verification:
   ```markdown
...
5. Archive superseded proposals if applicable, update SIGNALS.md, commit:
   `"[SpecTeam] align — D-{N}: {title} resolved ({proposer} proposed, {confirmer} confirmed)"`
...
3. Validate that `.spec/design/$ARGUMENTS` exists. If not, list available files and ask user to specify.
...
5. Move the file to `.spec/archive/{YYYYMMDD}/`.
...
8. `git add .spec/` and commit: `"[SpecTeam] archive — {code}/{filename} frozen"`
...
4. Scan source directories, compute current file hashes, compare with `last-sync.json`:
   ...
   - Deleted file → needs removal from `.spec/design/{code}/`
   - Unchanged → skip
5. If no changes → output "No source document changes. .spec/ is up to date." and stop.
...
1. Copy new/modified files to `.spec/design/{code}/`, preserve Spec header comment.
2. Remove deleted files from `.spec/design/{code}/`.
...
4. **Action Items verification**: For each modified file, check if it appears in any `resolved` divergence's source document action items where `{me}` has `⏳ Pending update` status:
   - Read `.spec/decisions/D-{N}.md`, locate the **[{me}] Change Instructions** block, extract the **Acceptance criterion** field (primary criterion) and **Required changes** (detail).
...
6. `git add .spec/design/{code}/ .spec/last-sync.json .spec/DIVERGENCES.md`
7. Commit: `"[SpecTeam] update — {code} source sync: +{N} ~{N} -{N}"`
