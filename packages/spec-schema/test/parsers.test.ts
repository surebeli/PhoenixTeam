...
test("modern signal fixtures parse and validate", () => {
  for (const state of ["clean-workspace", "conflicted", "resolved-pending-action-items", "fully-closed"]) {
    const result = parseSignals(readStateFile(state, "SIGNALS.md"));
    assertParseSuccess(result, `${state}/SIGNALS.md`);
    assert.equal(validate("signal", result.value).ok, true);
  }
});

test("modern index fixtures parse and validate", () => {
  for (const state of ["clean-workspace", "conflicted", "proposed-multi-party", "resolved-pending-action-items", "fully-closed"]) {
    const result = parseIndex(readStateFile(state, "INDEX.md"));
    assertParseSuccess(result, `${state}/INDEX.md`);
    assert.equal(validate("index-doc", result.value).ok, true);
  }
});
...
test("corrupt signals input yields PX-P004", () => {
  const text = readStateFile("conflicted", "SIGNALS.md").replace("[2026-04-24]", "[2026/04/24]");
  assertParseCode(parseSignals(text), "PX-P004");
});
...
test("corrupt index input yields PX-P001", () => {
  const text = readStateFile("clean-workspace", "INDEX.md").replace("_Generated by", "_Built by");
  assertParseCode(parseIndex(text), "PX-P001");
});